...
exports.getOnePublicity = function(name,callback){
  pubs.find({name:name},function(err,doc){
		if (doc.length != 0) {
      callback({'res':true,'data':doc});
    } else {
      callback({'res':false});
...
exports.addPublicity = function(name,category,period,price,date,callback){
  var newTaxi = new pubs({name:name,category:category,period:period,price:price,date:date});
...
exports.editPublicity = function(id,name,category,period,price,date,callback){
  pubs.update({_id:id},{name:name,category:category,period:period,price:price,date:date},function(err){
    if (err) {
