...
exports.profileAdmin = function(token,callback){
  drivers.find({_id:token},function(err,data){
    if(data.length != 0){
      var fname = data[0].fname;
      var lname = data[0].lname;
      ...
      var phone = data[0].phone;
      var picture = data[0].picture;
      callback({'res':true,'fname':fname,'lname':lname,
        'gender':gender,'dateN':dateN,'country':country,
...
exports.disableAccount = function(token,password,callback){
  drivers.find({_id:token},{password:1,salt:1},function(err,data){
    if(data.length != 0){
      var hash_db = data[0].password;
      var temp = data[0].salt;
      var hashed_password = crypto.createHash('sha512').update(temp + password).digest("hex");
...
exports.disableAccountBook = function(token,callback){
  drivers.find({_id:token},{_id:0,fname:1,lname:1,life:1},function(err,data){
    var msg = '';
    if(data.length != 0){
      msg = 'Driver ' + data[0].fname + ' ' + data[0].lname;
      if (data[0].life < 3) {
        ...
        drivers.update({_id:token},{$set:{account:false,connected:false}},{multi:true},function(err){
          if(err){
            callback({'res':false});
          }else{
            ...
            var date = datenew.getDate() + "/"
              ...
              + datenew.getMinutes() + ":"
              + datenew.getSeconds();
            var newReclamation = new reclamations({token:null,username:null,subject:'Driver account disabled',date:date,
              status:true,me:true,messages:{sender:true,message:msg,date:date}});
